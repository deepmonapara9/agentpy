...
class GmailImapParser:
    ...
    def debug_search_comparison(self,
        ...
        try:
            ...
            if from_email:
                ...
                emails_with_from = self.search_emails(
                    search_criteria_with_from)

            # Parse a few emails from the no_from search to see their from addresses
            sample_emails = []
            # Sample first 10
            for i, email_id in enumerate(emails_no_from[:10]):
                email_data = self.parse_single_email(
                    email_id, keep_unread=True, use_uid=True)
                if email_data:
    ...
    def _search_single_folder(self, folder: str, **kwargs) -> List[Dict]:
        ...
        try:
            ...
            if verbose:
                ...
                print(
                    f"Found {len(email_ids)} {unread_status}email(s) in {folder}")

            # Parse emails
            parsed_emails = []
            for email_id in email_ids:
                email_data = self.parse_single_email(email_id, kwargs.get(
                    'keep_unread', True), use_uid=True, verbose=verbose)
                if email_data:
    ...
    def _search_multiple_folders(self, **kwargs) -> List[Dict]:
        ...
        # Sort by timestamp
        try:
            final_emails.sort(key=lambda x: x.get(
        ...
        if verbose:
            print(
                f"Total unique emails found across folders: {len(final_emails)}")
    ...
    def debug_all_emails_from_sender(self, from_email: str, limit: int = 20) -> List[Dict]:
        ...
        try:
            ...
            print(
                f"DEBUG: Found {len(email_ids)} total emails from {from_email}")
            ...
            try:
                recent_emails.sort(key=lambda x: x.get(
                    'timestamp', ''), reverse=True)
            except:
                pass
        ...
        for folder in gmail_folders:
            try:
                ...
                if folder_emails:
                    ...
                    print(
                        f"  Parsed {len(folder_emails)} emails from {folder}")
                    for email in folder_emails:
                        print(
                            f"    UID: {email['uid']}, Subject: {email['subject']}")
